...
class Delphi(nn.Module):
    ...
    def __init__(self, config, embeddingsOnly=False):
        super().__init__()
    ...
    def forward(self, idx, age, targets=None, targets_age=None, validation_loss_mode=False):
        ...
        seq = torch.arange(idx.size(1), device=idx.device)
        eye_bool = (seq.view(1, 1, idx.size(1), 1) == seq.view(1, 1, 1, idx.size(1)))
        if targets is not None and self.config.mask_ties:
            ...
            attn_mask = attn_mask | ((attn_mask.sum(-1, keepdim=True) == 0) & eye_bool)
        attn_mask = attn_mask | (((idx == 0).view(idx.size(0), 1, 1, idx.size(1))) & eye_bool)
        attn_mask *= torch.tril(torch.ones(idx.size(1),idx.size(1), device=device))[None,None,:,:] > 0 #self.transformer.h[0].attn.bias[:,:,:idx.size(1),:idx.size(1)] > 0
